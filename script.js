    ...
    centroidData.features.forEach(feature => {
        ...
        function displayPolarChart() {
            ...
            new Chart(ctx, {
                type: 'polarArea',
                data: {
                    labels: ['N', 'NE', 'E', 'SE', 'S', 'SW', 'W', 'NW'], // Ensure correct order
                    datasets: [{
                        label: 'Population by Direction',
                        data: directionData,
                        backgroundColor: [
                            '#FF6384', '#36A2EB', '#FFCE56', '#4BC0C0',
                            '#9966FF', '#FF9F40', '#C9CBCF', '#32CD32'
                        ],
                    }]
                },
                options: {
                    ...
                    plugins: {
                        ...
                        datalabels: {
                            ...
                            align: 'end', // Position labels inside the chart
                            anchor: 'center', // Center labels above data
                            formatter: (value, context) => context.chart.data.labels[context.dataIndex] // Display label inside the chart
                        }
                    },
                    scales: {
                        r: {
                            startAngle: -22.5 // Slight counterclockwise rotation
                        }
                    }
                },
                plugins: [ChartDataLabels] // Ensure DataLabels plugin is loaded
            });
        }
...
function displayPopulationSummary(totalPopulation, racePopulation) {
    ...
    let summaryHtml = `
        <h3>Population Summary (Inside Created Boundary)</h3>
        <p><strong>Total Estimated Population (2020 Census):</strong> ${totalPopulation}</p>
        </div>
    ...
    setTimeout(() => {
        ...
        // **Generate Directional Population Data**
        let directionData = generateDirectionPopulationData();
        

        // **Event Listener for Toggle Buttons**
        document.getElementById("toggle-race").addEventListener("click", () => {
            raceChart.canvas.style.display = "block";
            directionChart.canvas.style.display = "none";
        });

        document.getElementById("toggle-direction").addEventListener("click", () => {
            raceChart.canvas.style.display = "none";
            directionChart.canvas.style.display = "block";
        });
...
function displayPolarChart() {
    ...
    // Make the panel visible once data is available
    document.getElementById("polar-chart-panel").style.display = "block";
